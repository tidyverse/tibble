...
We resolve this by first defining with `[[`, then colum-wise subsetting with `[`, then row-wise subsetting, then the composition of both.
...
<!-- HW: the failure with mean needs a better error. -->

Missing and out-of-bounds (OOB) values of `j` return `NULL` with a warning or error:
...
<!-- HW: why is x[[NA]] an OOB error and not a type error? -->

OOB access only generate a warning because a common existing idioms is to check for the absense of a column with `is.null(df$x)`.

Non-integer numeric values produce an error:

```{r, double-bracket-numeric, dftbl = TRUE}
df[[1.5]]
```
...
<!-- I removed the examples here because I think the principle is so simple -->
