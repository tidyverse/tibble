...
`x[[j]] <- a` is equivalent to restoring to the original class after assigning to `unclass(x)`:
...
`[[<-` supports changing the type of a column.

```{r double-bracket-assign-supports-null, dftbl = TRUE}
with_df(df[[1]] <- LETTERS[4:1])
with_df(df[[2]] <- 4:1)
with_df(df[[3]] <- 4:1)
with_df2(df2[[1]] <- 4:1)
with_df2(df2[[2]] <- 4:1)
...
`x[j] <- a` is equivalent to `x[[jj]] <- ...` in a loop (indexed by position), keeping only columns that are assigned non-NULL values.
...
for (jj in seq_along(j)) x[[ j[[jj]] ]] <- vec_recycle(a[[jj]], nrow(x))
```

Adding, updating and removing columns may occur in arbitrary order.
To avoid index mismatches, an actual implementation might look more like this:
...
df[5:7, ]
df[6, ]
with_df(df[5, ] <- list(0, "x", list(NULL)))
with_df(df[5:7, ] <- list(0, "x", list(NULL)))
with_df(df[6, ] <- list(0, "x", list(NULL)))
df[-5, ]
df[-(5:7), ]
df[-6, ]
with_df(df[-5, ] <- list(0, "x", list(NULL)))
