...
```{r double-bracket-retains-size, dftbl = TRUE, include = FALSE}
vec_size(df[[1]])
vec_size(df[[3]])
vec_size(df2[[1]])
vec_size(df2[[2]])
...
```{r dollar-equivalent-to-subset-detail, dftbl = TRUE, include = FALSE}
...
```{r bracket-missing-i-detail, dftbl = TRUE, include = FALSE}
...
```{r bracket-always-returns-tibble-drop-detail, dftbl = TRUE, include = FALSE}
...
Applies `vec_slice(x[[j_1]], i) <-` to all columns.[^row-assign-symmetry]
