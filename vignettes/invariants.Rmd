...
`a` is recycled to the same size as `df` so must have size `nrow(x)` or 1.
...
`x[j] <- a` is equivalent to `x[[j[[1]]] <- a[[1]]`, `x[[j[[2]]]] <- a[[2]]`, ....
`a` is recycled to the same length as `j`.

```{r, dftbl = TRUE}
with_df(df[1:2] <- list(1))
with_df(df[1:2] <- list(1, 2))
```
...
If `a` contains `NULL` values, the corresponding columns are removed *after* updating. <!-- HW: why is _after_ signficant? -->
