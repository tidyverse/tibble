...
<!-- HW: This feels like too much detail: maybe after the first example it could just show all.equal(df1, .subset2[[1]])` -->
...
<!-- TODO: should x[[NA_character_]] error? -->

### Definition of `x$name`

`x$name` is equal to `x[["name"]]`.
Unlike data frames, tibbles do not partially match.
...
`j` is first converted to an integer vector by `vectrs::vec_as_index()`. This ensures that `j` is a logical, numeric, or character, and recycles scalar logicals.

Then `x[c(j_1, j_2, ..., j_n)]` is equvalent to `tibble(x[[j_1]], x[[j_2]], ..., x[[j_3]])` with names set to `names(x)[j]`.
