...
<!-- TODO: x[[NA]] should be a type error -->
<!-- TOOD: failure with mean needs a better error. -->
...
Character OOB access generates a warning rather than an error because a common existing idiom is to check for the absence of a column with `is.null(df$x)`.
...
<!-- TODO: should x[[NA_character_]] throw an error? -->
...
`j` must be a numeric or character vector, or a logical vector with length 1 or `nrow(x)`.

`j` is converted to an integer vector by `vectrs::vec_as_index(j, nrow(x), names = names(x))`. Then `x[c(j_1, j_2, ..., j_n)]` is equvalent to `tibble(x[[j_1]], x[[j_2]], ..., x[[j_3]])``.
...
If values of `j` are repeated, then ???
...
`i` is converted to an integer vector by `vectrs::vec_as_index(i, nrow(x))`.
...
`drop = TRUE` has no effect when not selecting a single column:
