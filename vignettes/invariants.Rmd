...
The [subsetting](articles/subset.html) and [subassignment](articles/subassign.html) articles give a comprehensive overview.
...
### `x[[j]]` is equivalent to `unlist(x[j])`.

```{r double-bracket-equivalent-to-unlist-single-bracket, dftbl = TRUE}
df[[1]]
unlist(df[1])
df[[3]]
unlist(df[3])
...
```{r double-bracket-equivalent-to-unlist-single-bracket-update, dftbl = TRUE}
with_df(df[[1]] <- df[[3]])
with_df(df[[1]] <- df2[[1]])
with_df(df[[1]] <- df2[[2]])
...
### `x[[j]]` always returns an object of length `nrow(x)`.

FIXME.

Corollary: `x[[i, j]]` always returns an object of length 1 (by definition of `x[[1, j]]`).

### `[` always returns a tibble.

```{r bracket-always-returns-tibble, dftbl = TRUE}
df[, 1]
df[1, ]
df[1, 1]
df[, 1, drop = FALSE]
...
### Dimensions are defined by i and j.

The dimensions after `[` and `[[` are given by the length of `vec_as_index(i)` and `vec_as_index(j)`.
...
- Multiple columns require a data frame at the rhs
