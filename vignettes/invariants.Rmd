...
We also show examples with hierarchical columns containting a data frame or a matrix where needed:
...
These function take an assignment expression, execute it on a fresh copy of the data, and return the data for printing.
...
We resolve this by first defining column-wise subsetting, then row-wise subsetting and the composition of both.
...
`[[<-` requires scalar `vec_as_index(j)` (with the exception of new names, see below).
...
Accessing `NA` columns returns `NULL`.
...
# New columns
tibble:::vec_as_new_col_index("x", df, list(0))
...
with_df(df[0:2, ] <- list(0, "x", list(NULL)))
with_df(df[-2, ] <- list(0, "x", list(NULL)))
with_df(df[-1:2, ] <- list(0, "x", list(NULL)))
with_df(df[NA_integer_, ] <- list(0, "x", list(NULL)))
with_df2(df2[NA_integer_, ] <- list(0, "x", list(NULL)))
with_df(df[c(FALSE, TRUE, TRUE, FALSE), ] <- list(0, "x", list(NULL)))
with_df(df[TRUE, ] <- list(0, "x", list(NULL)))
with_df(df[NA, ] <- list(0, "x", list(NULL)))
```

### Corollary

For logical vectors, out-of-bounds indexing and recycling of size > 1 are not supported.
(Charecter and numeric vectors are exceptions that are discussed below.)
...
df[as.character(-(3:5)), ]
df[NA_character_, ]
with_df(df[as.character(1:3), ] <- 0)
with_df(df[as.character(-(1:3)), ] <- 0)
with_df(df[as.character(3:5), ] <- 0)
with_df(df[as.character(-(3:5)), ] <- 0)
...
with_df(df[2:3, "x"] <- list(0))
with_df(df[2:3, "c"] <- NULL)
```
