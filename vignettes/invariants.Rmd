...
NB: `x[j][[jj]]` is equal to `x[[ j[[jj]] ]]`, in particular `x[j][[1]]` is equal to `x[[j]]` for scalar numeric or integer `j`.
...
`i` must be a numeric vector, or a logical vector of length `nrow(x)` or 1.
...
except that OOB values generate warnings instead of errors:
