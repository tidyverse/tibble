...
df[[NA]]
df[[NA_character_]] # FIXME
df[[NA_integer_]] # FIXME
df[[-1]]
df[[4]]
df[[1.5]]
df[[Inf]]
...
`j` is converted to an integer vector by `vctrs::vec_as_index(j, ncol(x), names = names(x))`.
...
[^subset-extract-commute]: NB: `x[j][[jj]]` is equal to `x[[ j[[jj]] ]]`, in particular `x[j][[1]]` is equal to `x[[j]]` for scalar numeric or integer `j`.
...
For efficiency and backward compatibility, `i` is converted to an integer vector by `vctrs::vec_as_index(i, nrow(x))` first.
...
```{r bracket-i-j-equivalent-to-j-subset-then-i, dftbl = TRUE, include = eval_details, eval = eval_details}
df[1, 1]
df[1, ][1]
...
`x[[i, j]]` is equal to `x[i, ][[j]]`.[^bracket2-flip]
...
NB: `vctrs::vec_size(x[[i, j]])` always equals 1.
...
`x[[j]] <- a` corresponds to `x <- tibble(x[[j_1]], x[[j_2]], ..., a, ..., x[[j_n]])`.
...
```{r double-bracket-assign-requires-scalar-j-index-error, dftbl = TRUE}
with_df(df[[TRUE]] <- 0)
with_df(df[[1:3]] <- 0)
with_df(df[[c("n", "c")]] <- 0)
with_df(df[[FALSE]] <- 0)
...
`vec_size(a)` must be `nrow(x)` or 1.
...
```{r dollar-equivalent-to-subset-assign-detail, dftbl = TRUE}
...
The type of an existing column can be changed, also across the principal column types:
...
[^column-assign-symmetry]: `$` behaves almost completely symmetrical to `[[` when comparing subsetting and subassignment.
...
`x[j] <- a` is equivalent to performing `x[[j_1]] <- a[[1]]`, `x[[j_2]] <- a[[2]]` etc.
...
Just like column updates, `[<-` supports changing the type of an existing column, also across the principal column types:
...
```{r bracket-assign-recycles, dftbl = TRUE, include = eval_details, eval = eval_details}
...
```{r bracket-empty, dftbl = TRUE, include = eval_details, eval = eval_details}
with_df(df[] <- df)
with_df(df[] <- df[1, ])
with_df(df[] <- df[1:2, ])
with_df(df[] <- df2)
...
[^row-assign-symmetry]: `x[i, ]` are symmetrical for subset and subassignment.
...
Subassignment to `x[i, j]` is much stricter for tibbles than for data frames.
`x[i, j] <- a` can update only existing columns, it cannot create or remove columns.
...
NB: `vctrs::vec_size(a)` must equal 1.
Unlike `x[i, ] <-`, `x[[i, ]] <-` is not valid.
