...
`x[[j]]` always returns and requires an object of size `nrow(x)`.
...
`x[[i, j]]` is equivalent to `x[i, j][[1]]`.
...
`x[[i, j]]` and `x[i, j][[1]]` can be used interchangeably on the LHS or RHS of an assignment for the four principal column types:
...
with_df(df[1, 1][[1]] <- df[2, 1][[1]])
with_df(df[[1, 3]] <- df[[2, 3]])
with_df(df[1, 3][[1]] <- df[2, 3][[1]])
with_df2(df2[[1, 1]] <- df2[[2, 1]])
with_df2(df2[1, 1][[1]] <- df2[2, 1][[1]])
with_df2(df2[[1, 2]] <- df2[[2, 2]])
with_df2(df2[1, 2][[1]] <- df2[2, 2][[1]])
```
...
Selects all rows from the columns given by `vec_as_index(j)`.
...
`x[]` is equivalent to `x` (with [rlib/vctrs#547](https://github.com/r-lib/vctrs/issues/547)).
...
Keeps elements given by `vec_as_index(i)` for all columns.
...
```{r bracket-i-empty, dftbl = TRUE}
df[,]
```

Out-of-bounds indexing (and recycling of logical vectors of size > 1) is not supported, but for compatibility reasons only a warning is issued for indexing beyond the number of rows.

```{r bracket-i-oob, dftbl = TRUE}
dim(df[c(TRUE, FALSE), ])
dim(df[c(TRUE, FALSE, TRUE, FALSE, TRUE), ])
dim(df[5, ])
```
