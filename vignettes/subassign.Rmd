...
same_as_tbl_code <- function(df_code, tbl_code) {
  ...
  tbl <- new_tbl()
  same_as_tbl(
    rlang::eval_bare(rlang::parse_expr(df_code)),
    rlang::eval_bare(rlang::parse_expr(tbl_code))
  )
}

same_as_tbl <- function(df, tbl) {
  df <- try(df, silent = TRUE)
  tbl <- try(tbl, silent = TRUE)
  
  if (is.data.frame(df)) {
    if (is.data.frame(tbl)) {
      identical(df, as.data.frame(tbl))
    } else {
      FALSE
    }
  } else {
...
In the following, if the results are identical, only the tibble result is shown, as in the example below.
