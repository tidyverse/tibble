...
Updating parts of a column extracted by `$` is responsibility of the column vector. Tibbles can't control what happens after `$` has returned.
...
with_df(df$a[1:2] <- 4:3)
with_df(df$v[1:2] <- vctrs::new_vctr(4:3))
with_df(df$v[1:2] <- vctrs::new_vctr(letters[4:3]))
```
