...
with_df <- function(code) {
  ...
  rlang::expr_print(full_code)
  rlang::eval_tidy(full_code)
}

with_tbl <- function(code) {
  code <- rlang::enexpr(code)
...
This function takes an assignment expression and executes it on a fresh copy of the data, printing what's executed.

```{r}
with_df({})
with_df(df$a <- rev(df$a))
```
