...
pillar_shaft.latlon <- function(x, ...) {
  out <- format(x)
  out[is.na(x)] <- NA
  new_pillar_shaft_simple(out, align = "left", na_indent = 5)
}
...
pillar_shaft.latlon <- function(x, ...) {
  ...
  deg_min <- format(x)
  deg_min[is.na(x)] <- style_na("NA")
  new_pillar_shaft(
    list(deg = deg, deg_min = deg_min),
    width = get_max_extent(deg_min),
    min_width = get_max_extent(deg),
...
test_that("latlon pillar matches known output", {
  expect_known_display(pillar_shaft(data$loc), file = "latlon.txt")
})
```
...
library(testthat)
test_that("latlon pillar matches known output", {
  expect_known_display(pillar_shaft(data$loc), file = "latlon.txt", crayon = FALSE)
})
```
...
expect_known_latlon_display <- function(x, file_base) {
  quo <- rlang::quo(pillar_shaft(x))
  expect_known_display(
    !! quo, file = paste0(file_base, ".txt")
  )
  expect_known_display(
    !! quo, file = paste0(file_base, "-bw.txt"), crayon = FALSE
  )
}
```
