...
data frame. See
[`tribble()`](https://tibble.tidyverse.org/dev/reference/tribble.md) for
an easy way to create an complete data frame row-by-row. Use
[`tibble_row()`](https://tibble.tidyverse.org/dev/reference/tibble.md)
to ensure that the new data has only one row.
...
- ...:
  ...
  Values can be defined only for columns that already exist in `.data`
  and unset columns will get an `NA` value.
