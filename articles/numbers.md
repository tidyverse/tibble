...
Tibbles print numbers with three significant digits by default,
...
for an overview over the differences. This display works for many, but
...
The easiest way to customize the display of numbers and other data in a
tibble is to define options. See
...
This changes the display of all columns. Read on to see how too specify
display options on a column-by-column basis.

## Per-column number formatting

The new [`num()`](https://tibble.tidyverse.org/reference/num.md)
constructor allows creating vectors that behave like numbers but allow
customizing their display. Below a few examples are shown, see
[`?num`](https://tibble.tidyverse.org/reference/num.md) for a
comprehensive overview. Similarly,
[`char()`](https://tibble.tidyverse.org/reference/char.md) allows
...
#> 1  800.5000    800.5  800.50    8.05     1 123   m A B C D E… V W X Y Z
#> 2  900.5000    900.5  900.50    9.05    10   1.23  A B C D E… V W X Y Z
#> 3 1000.5000   1000.5 1000.50   10.0    100  12.3   A B C D E… V W X Y Z
#> 4 1100.5000   1100.5 1100.50   11.0   1000 123     A B C D E… V W X Y Z
#> 5 1200.5000   1200.5 1200.50   12.0  10000   1.23k A B C D E… V W X Y Z
```

The pillar package that is responsible for the display of tibbles tries
hard to get the number display right, however it is impossible to
accommodate all use cases. Whenever the default formatting does not suit
the application,
[`num()`](https://tibble.tidyverse.org/reference/num.md) or
[`char()`](https://tibble.tidyverse.org/reference/char.md) allow
redefining the formatting for individual columns. The formatting
survives most data transformations.

## Rule-based number formatting

Currently, formatting must be applied manually for each column. The
...
Formatting numbers is useful for presentation of results. If defined
...
final presentation (plots, tables, …).

### Arithmetics

When applying arithmetic operations on numbers created by
[`num()`](https://tibble.tidyverse.org/reference/num.md), the result
...
[`num()`](https://tibble.tidyverse.org/reference/num.md) can be applied
repeatedly, the last setting wins.
...
The [`var()`](https://rdrr.io/r/stats/cor.html) function is one of the
...
The [`median()`](https://rdrr.io/r/stats/median.html) function is worse,
it breaks for [`num()`](https://tibble.tidyverse.org/reference/num.md)
objects:
...
One way to recover is to apply
[`num()`](https://tibble.tidyverse.org/reference/num.md) to the result:
...
[`var()`](https://rdrr.io/r/stats/cor.html), or even overwrite the base
implementation. Note that this pattern is still experimental and may be
subject to change:
...
The `make_restore()` function defined below is a function factory that
