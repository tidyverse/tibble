...
check_names_df.character <- function(j, x) {
  ...
  if(any(is.na(pos))){
    names <- j[is.na(pos)]
    stopc("Unknown column(s) ", format_n(names))
  }
  pos
}
...
check_names_df.numeric <- function(j, x) {
  ...
  if (any(non_integer)) {
    stopc("Invalid non-integer column index(es): ", format_n(j[non_integer]))
  }
  neg_too_small <- (j < -length(x))
  if (any(neg_too_small)) {
    stopc("Invalid negative column index(es): ", format_n(j[neg_too_small]))
  }
  pos_too_large <- (j > length(x))
  if (any(pos_too_large)) {
    stopc("Invalid column index(es): ", format_n(j[pos_too_large]))
  }
