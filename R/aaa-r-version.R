...
# Switching R versions is useful: LLDB debugging only works with non-notarized versions of R (e.g. R-devel) on macOS,
# binary packages (e.g. arrow) are available only for stable versions.
# This code cleans up if a mismatch in R versions is detected, via a hidden src/version.txt file.
#
# This code is run directly, not in a function, to support both R CMD INSTALL and pkgload::load_all().
...
# - For a package installed via R CMD INSTALL,
#   the file is sourced manually in ./configure (which is always run) and does the right thing
