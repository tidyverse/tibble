...
#'
#' The general ethos is that tibbles are lazy and surly: they do less and complain more than base [data.frame]s.
#' This forces problems to be tackled earlier and more explicitly,
#' typically leading to code that is more expressive and robust.
#'
#' @section Properties of `tbl_df`:
#'
#' Objects of class `tbl_df` have:
#' * A `class` attribute of `c("tbl_df", "tbl", "data.frame")`.
#' * A base type of `"list"`, where each element of the list has the same [vctrs::vec_size()].
...
#'   any row names that might be stored there are ignored by most tibble methods.
#'
#' @section Behavior of `tbl_df`:
#'
#' How default behaviour of tibbles differs from that of [data.frame]s, during creation and access:
#'
#' * Column data is not coerced.
#'   A character vector is not turned into a factor.
...
#'   Empty and duplicated column names are strongly discouraged, but the user must indicate how to resolve.
#'   Read more in [vctrs::vec_as_names()].
#' * Row names are not added and are strongly discouraged, in favor of storing that info as a column.
#'   Read about in [rownames].
#' * `df[, j]` returns a tibble; it does not automatically extract the column inside.
#'   `df[, j, drop = FALSE]` is the default.
#'   Read more in [subsetting].
#' * There is no partial matching when `$` is used to index by name.
#'   `df$name` for a nonexistent name generates a warning.
#'   Read more in [subsetting].
#'
...
#' The goal is to convey as much information as possible, in a concise way, even for large and complex tibbles.
#' Read more in [formatting].
#'
#' @name tbl_df-class
...
`names<-.tbl_df` <- function(x, value) {
  # workaround for RStudio v1.1, which relies on the ability to set data.frame names to NULL
