...
set_unique3_names <- function(x, quiet = FALSE) {
  x <- set_minimal_names(x)
  new_names <- unique3_names(names(x), quiet = quiet)
  set_names(x, new_names)
}

universal3_names <- function(name, quiet = FALSE) {
  unique3_names(name, quiet = quiet, transform = make_syntactic3)
}

set_universal3_names <- function(x, quiet = FALSE) {
  x <- set_minimal_names(x)
  new_names <- universal3_names(names(x), quiet = quiet)
  set_names(x, new_names)
}
