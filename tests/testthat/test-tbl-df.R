...
test_that("[.tbl_df is careful about column indexes (#83)",{
  ...
  expect_error(
    foo[0.5], "Invalid non-integer column index: 0.5",
    fixed = TRUE
  )
  expect_error(
    foo[1:5], "Invalid column indexes: 4, 5",
    fixed = TRUE
  )
  expect_error(
    foo[-1:1], "mixed with negative")
  expect_error(
    foo[c(-1, 1)],
    "mixed with negative",
    fixed = TRUE
  ...
  expect_error(
    foo[-4], "Invalid negative column index: -4",
    fixed = TRUE
  )
  expect_error(
    foo[c(1:3, NA)], "NA column indexes not supported",
    fixed = TRUE
  )
...
test_that("[.tbl_df is careful about column flags (#83)",{
  ...
  expect_error(
    foo[c(TRUE, TRUE)],
    "Length of logical index vector must be 1 or 3, got: 2",
    fixed = TRUE
  )
  expect_error(
    foo[c(TRUE, TRUE, FALSE, FALSE)],
    "Length of logical index vector must be 1 or 3, got: 4",
    fixed = TRUE
  )
  expect_error(
    foo[c(TRUE, TRUE, NA)],
