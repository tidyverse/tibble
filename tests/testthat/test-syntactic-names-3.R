context("universal3_names")

# make_syntactic3 -------------------------------------------------------------
expect_syntactic <- function(name, exp_syn_name) {
  expect_identical(
    syn_name <- make_syntactic3(name),
    exp_syn_name
  )
...
test_that("make_syntactic3(): reserved words", {
  expect_syntactic(
...
test_that("make_syntactic3(): underscore", {
  expect_syntactic(
    c( "_",  "_1",  "_a}"),
    c("._", "._1", "._a.")
  )
...
test_that("make_syntactic3(): dots", {
  expect_syntactic(
    c(".", "..",  "...", "...."),
    c(".", "..", "....", "....")
  )
...
test_that("make_syntactic3(): number", {
  expect_syntactic(
      c(   "0",    "1",    "22",    "333"),
      c("...0", "...1", "...22", "...333")
  )
...
test_that("make_syntactic3(): dot dot then number", {
  expect_syntactic(
    c( "..0",  "..1",  "..22",  "..333"),
    c("...0", "...1", "...22", "...333")
  )
...
test_that("make_syntactic3(): dot dot dot then number", {
  expect_syntactic(
    c("...0", "...1", "...22", "...333"),
    c("...0", "...1", "...22", "...333")
  )
...
test_that("make_syntactic3(): dot dot dot dot then number", {
  expect_syntactic(
    c("....0", "....1", "....22", "....333"),
    c("....0", "....1", "....22", "....333")
  )
...
test_that("make_syntactic3(): dot dot dot dot dot then number", {
  expect_syntactic(
    c(".....0", ".....1", ".....22", ".....333"),
    c(".....0", ".....1", ".....22", ".....333")
  )
...
test_that("zero-length input", {
  expect_equal(universal3_names(character()), character())
})

test_that("universal names are not changed", {
  expect_equal(universal3_names(letters), letters)
})

test_that("universal3_names() pass checks for minimal, unique, and universal", {
  x <- c(NA, "", "x", "x", "a1:", "_x_y}")
  x_syn <- universal3_names(x)
...
test_that("universal3_names() is idempotent", {
  x <- c(NA, "", "x", "x", "a1:", "_x_y}")
  expect_identical(universal3_names(x), universal3_names(universal3_names(x)))
})
...
test_that("empty, dot becomes suffix, dot", {
  expect_equal(universal3_names(c("", ".")), c("...1", "."))
})
...
test_that("quiet = TRUE", {
  expect_message(set_universal3_names(set_names(1, ""), quiet = TRUE), NA)
})

test_that("non-universal names", {
  out <- set_universal3_names(set_names(1, "a b"))
  expect_equal(names(out), "a.b")

  expect_equal(universal3_names("a b"), "a.b")
})
