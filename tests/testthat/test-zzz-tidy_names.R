...
test_that("unnamed input gives uniquely named output", {
  suppressMessages(out <- set_tidy_names(1:3))
...
test_that("dupes", {
  expect_equal(
    suppressMessages(tidy_names(c("a", "b", "a", "c", "b"))),
    c("a..1", "b..2", "a..3", "c", "b..5")
  )
})

test_that("empty", {
  expect_equal(suppressMessages(tidy_names("")), "..1")
})

test_that("NA", {
  expect_equal(suppressMessages(tidy_names(NA_character_)), "..1")
})

test_that("corner case", {
  expect_equal(suppressMessages(tidy_names(c("a..2", "a"))), c("a..2", "a"))
  expect_equal(suppressMessages(tidy_names(c("a..3", "a", "a"))), c("a..1", "a..2", "a..3"))
  expect_equal(suppressMessages(tidy_names(c("a..2", "a", "a"))), c("a..1", "a..2", "a..3"))
  expect_equal(suppressMessages(tidy_names(c("a..2", "a..2", "a..2"))), c("a..1", "a..2", "a..3"))
})

test_that("syntactic", {
  expect_equal(
    suppressMessages(tidy_names("a b", syntactic = TRUE)),
    make.names("a b")
  )
})
...
test_that("some syntactic + message (#260)", {
  expect_equal(
    suppressMessages(tidy_names(c("a b", "c"), syntactic = TRUE)),
    c(make.names("a b"), "c")
  )
})
